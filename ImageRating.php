...
class ImageRating extends SpecialPage {
	...
	function execute(){
		global $wgRequest, $IP, $wgOut, $wgUser, $wgMemc, $wgStyleVersion, $wgContLang, $wgVoteDirectory;
		...
		foreach($menu as $title=>$qs){
			if ($type!=$qs){
				$output.= "<p><a href=\"index.php?title=Special:ImageRating&type={$qs}" . (($category)?"&category={$category}":"") . "\">{$title}</a><p>";
			} else {
		...
		*/
		...
		if($numofpages>1){
			...
			if($page > 1){ 
				$output .= "<a href=\"index.php?title=Special:ImageRating&page=" . ($page-1) . "&type={$type}" . (($category)?"&category={$category}":"") . "\">prev</a> ";
			}
			...
			for($i = 1; $i <= $numofpages; $i++){
				if($i == $page){
				    ...
				    $output .="<a href=\"index.php?title=Special:ImageRating&page=$i&type={$type}" . (($category)?"&category={$category}":"") . "\">$i</a> ";
				}
			}
	
			if(($total - ($per_page * $page)) > 0){
				$output .=" <a href=\"index.php?title=Special:ImageRating&page=" . ($page+1) . "&type={$type}" . (($category)?"&category={$category}":"") . "\">next</a>"; 
			}
			$output .= "</div>";
		}
